...
export const advantages: Record<string, AdvantageDef> = {
  "Access to Natural Psychic Powers": {
    effect: "The character has a limited psychic capacity that allows him to use one Psychic Ability unconsciously. They don't know the origin of the power or have complete control over it, but they can use it whenever they needs to. Effects: The character can naturally use one specific Psychic Ability. They are not a true psychic and cannot use Psychic Points to acquire new abilities or to harness the power that they have. their psychic potential is not based on Willpower and does not require rolling dice, but automatically is Difficult (DIF). The chosen power can be of any level, but it cannot have a base requirement greater than Difficult. It can be used once per minute without the character suffering a penalty, but each additional use without the required rest inflicts 1 point of Fatigue. Spending additional points increases the natural psychic potential to Very Difficult (VDF) and Absurd (ABS), respectively.",
    Cost: [1, 2, 3],
